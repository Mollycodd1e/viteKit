    ...
    return (
        <>
            ...
            <div style={{margin: '20px 0'}}>
                <Button as='button' variant='sokolniki' onClick={() => setIsIconOpen((prev) => !prev)}>ТУТ ВСЕ
                    ИКОНКИ</Button>
            </div>
            ...
            <Modal isOpen={isIconOpen} createPortalObj={{domNode: document.body}}
                   emitIsOpen={() => setIsIconOpen((prev) => !prev)} isClickOutside={true}>
                <div style={{
                    background: "white",
                    padding: '40px',
                }} onClick={(e) => e.stopPropagation()}>
                    {Array.isArray(Object.keys(iconTypes)) ? Object.keys(iconTypes).map((e, i) => {
                        return <div style={{width:'100%', display: 'flex', justifyContent: 'space-between'}}>
                            <NewIcon key={i} name={e} size='16'/>
                            {e ?? ''}
                        </div>
                    }) : null}
                </div>
